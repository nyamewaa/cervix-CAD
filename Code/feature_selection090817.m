...
% clc,clear
%%
labels_biop=cellstr(Pathologybinary([1:134],:));%read in excell sheet via0225182.xlsx
%select sheet which say VIA features gabor. Select pathology binary column and read in as a
%column vector
labels_biop1=labels_biop';
%read in matrix of VIA features as a numeric matrix from same excel sheet.
% via_color=color_features;
% via_text=all_stats;
% via=[via_color,via_text];
% vili=la_liga;
% selected_via=via(:,[11,16,31,3,2,28]);
% selected_vili=vili(:,[

% la_liga(:,[1:65]);
% lesion1=lesion(:,66);
% labels_biop=table2array(viafeature([1:134],6));
data=via0225182S6;% [all_stats,color_features];%([1:134],:); %viaaftervili([1:98],:);
...
%% feature names
names={'contrast','correlation','energy','homogeneity','gab_mean','gab_med','gab_var','viaa_level_lugol','viaa_mean_img','viaa_median_img','viaa_mode_img','viaa_var_img','viab_level_lugol','viab_mean_img',...
...
optimal_ID=featureIdxSortbyP(1:72);
...
fsCVfor18 = fs1(historyCV.In(20,:));
%order them
[orderlist,ignore] = find( [historyCV.In(1,:); diff(historyCV.In(1:20,:) )]' );
